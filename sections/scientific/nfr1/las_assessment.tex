...
When evaluating the model on the testing dataset from the LibriSpeech dataset, which contains 2,620 examples, it achieves a WER of 24\% as seen in Table \ref{table:test_100_chars}.
...
Looking at the results of other speech recognition models in Table \ref{table:result_comparison}, our experiments with the LAS model didn't perform as well as others. An explanation for the higher error rate is that we did not train our model with the entire LibriSpeech dataset.
