\subsubsection{FR03: Investigation of the architecture of an attention-based sequence to sequence model.} % corrected 93
...
\paragraph{Bidirectional LSTM RNN}
...
The LAS model stacks 3 pBLSTMs above the BLSTM input layer. This decreases the time resolution $2^3 = 8$ times, allowing the attention model to derive important information from an input sequence with fewer time steps. An illustration of the $Listen$ operation is given in Figure \ref{las_model}
...
where $\phi$ and $\psi$ are MLP networks. $c_i$ can be interpreted as a collection of weighted features of h. Figure \ref{las_model} illustrates the operation $AttendAndSpell$.
