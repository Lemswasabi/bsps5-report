...
Before training an end-to-end ASR model, audio data have to be preprocessed. The first task is to extract \textcolor{red}{low-level} features from the data which describe\sout{s} natural language expressions and exclude\sout{s} background noises and emotions. Speech processing is an important process when creating ASR systems. For a long period, Mel-Frequency Cepstral Coefficients (MFCCs) were \textcolor{red}{the most} popular features. However, Mel-Filterbanks are recently getting more prominent \textcolor{red}{with applications in deep learning}. Computing both features contain the same approach where both of them calculate filterbanks\textcolor{red}{, but MFCCs include an additional step of applying Discrete Cosine Transform (DCT) to decorrelate the features. While this is preferred for most of the traditional machine learning algorithms, in DNNs it is not necessary since they are less susceptible to highly correlated input \cite{fayek2016}. Note that DCT is a linear transform, therefore, it discards some non-linear information in speech signals, which is not desirable in DNNs}. \sout{and when adding a few more steps MFCCs are retrieved. MFCCs were preferred over filterbanks because some Machine Learning algorithms were more susceptible to highly correlated input found in filterbanks. With the rise of Deep Learning based speech systems, filterbanks are favored since deep Neural Networks are less susceptible to highly correlated input.}\\
