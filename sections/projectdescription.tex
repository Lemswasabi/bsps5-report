...
\subsubsection{Technical} The technological aspects which are covered in this project are feature extraction, reusing an existing implementation of an attention-based sequence-to-sequence ASR model and benchmarking its performance using different configurations and hyperparameters.\\
...
\textbf{HPC environment.} High-performance computing (HPC) is the capability to process and compute large amounts of data at a fast pace. Implementations of HPC are often referred to as \textit{supercomputers} which consists of many connected computing servers called \textit{nodes}. These nodes work in parallel to compute tasks faster.\cite{hpc}
